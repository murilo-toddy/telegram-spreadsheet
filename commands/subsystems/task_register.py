...
def system(update: Update, ctx: CallbackContext) -> int:
    ...
    if system == "ele":
        ...
        update.message.reply_text(
            "Sistema não encontrado", reply_markup=ReplyKeyboardRemove()
        )
...
def get_active_projects() -> str:
    ...
    projects = [
        f"{index+1} - {row[0]}"
        for index, row in enumerate(row for row in data if row[0])
    ]
...
def subsystem(update: Update, ctx: CallbackContext) -> int:
    ...
    update.message.reply_text(
        reply_text, reply_markup=ReplyKeyboardRemove(), parse_mode=ParseMode.HTML
    )
    return PROJECT
...
def project(update: Update, ctx: CallbackContext) -> int:
    ...
    try:
        ...
        new_task["task"]["project"] = new_task["proj"][project_number - 1].split(" - ")[
            1
        ]
...
def task(update: Update, ctx: CallbackContext) -> int:
    ...
    update.message.reply_text(
        "Forneça uma estimativa (0 - 10) para a dificuldade desta tarefa"
    )
    return DIFFICULTY
...
def difficulty(update: Update, ctx: CallbackContext) -> int:
    ...
    update.message.reply_text(
        "Forneça uma estimativa de tempo (em semanas) para a realização desta tarefa"
    )
    return DURATION
...
def documents_question(update: Update, ctx: CallbackContext) -> int:
    ...
    if answer == "sim":
        update.message.reply_text(
            "Forneça o link para o(s) documento(s)", reply_markup=ReplyKeyboardRemove()
        )
    return DOCUMENTS
...
def confirmation(update: Update, ctx: CallbackContext) -> int:
    ...
    if answer == "sim":
        add_task_to_sheet()
        update.message.reply_text(
            "Tarefa adicionada com sucesso", reply_markup=ReplyKeyboardRemove()
        )
    else:
        update.message.reply_text(
            "Processo cancelado", reply_markup=ReplyKeyboardRemove()
        )
    return ConversationHandler.END
...
register_handler = ConversationHandler(
    ...
    states={
        ...
        DOC_QUESTION: [
            MessageHandler(Filters.text & ~(Filters.command), documents_question)
        ],
